...
- **Check** (:func:`cmd_check`, the ``shinro check`` verb): construct the
  component through its factory and print the inferred trace contract —
  "loads? constructs?".
...
Run (also reachable as ``shinro check`` / ``shinro trace``; ``scripts/
...
def cmd_check(config_path: str) -> int:
    """Construct a registered component and report its trace contract.

    The "loads? constructs?" gate: instantiate the config through its factory
    (which validates the config schema and fills plant-derived fields) and
    report the method the tracer will call. Does not trace.
    """
    loaded = _load_or_die(config_path)
    if loaded is None:
        return EXIT_USAGE
    ...
    contract = infer_contract(component)
    args = ", ".join(contract.input_names)
    print(f"OK: {ctype} ({category}) constructs — {contract.method_name}({args})")
    return EXIT_OK
