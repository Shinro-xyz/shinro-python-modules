...
traj_config = f"samples/trajectories/base_{TRAJECTORY}.toml"
...
sim = RobotSim(str(resolve_config_path("samples/robot_config.toml")), xml_string=xml, assets=assets)
sim.reset()

# ── Controller ───────────────────────────────────────────────────────────
ctrl_config = f"samples/controllers/{CONTROLLER}_base.toml"
ctrl_path = str(resolve_config_path(ctrl_config))
base_ctrl = ControllerFactory(ctrl_path).create()
CTRL_LABEL = CONTROLLER.upper()

# ── Estimator ────────────────────────────────────────────────────────────
base_observer = EstimatorFactory(str(resolve_config_path("samples/estimators/luenberger_base.toml"))).create()
