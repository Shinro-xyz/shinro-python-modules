...
kf  = EstimatorFactory("samples/estimators/kalman_base.toml").create()
...
and solve for you. shinro itself ships **no** configs: the repo's `samples/`
directory holds example configs to copy from, and the catalog in
[`components.md`](./components.md) lists every registered name and its config.

Config paths resolve against your **current working directory** (or an absolute
path) — never against a packaged copy. Run commands from your project root, or
use absolute paths; a path that doesn't resolve is a loud `FileNotFoundError`,
...
estimator, trajectory — as data. `samples/scenarios/cartpole_balance.toml`
is a complete MuJoCo-free example in the repo (run it from the repo root):

```python
from shinro import ScenarioFactory

scenario = ScenarioFactory("samples/scenarios/cartpole_balance.toml").build()
...
[trajectory]
type = "waypoints"
config = "samples/trajectories/cartpole_upright.toml"
```
...
```bash
shinro-compile samples/scenarios/cartpole_balance.toml --out build/scenario
```
