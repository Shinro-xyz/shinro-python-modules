...
this script reads the build manifest (``libbase.manifest.json``), hashes the
...
Run: ``python3 scripts/stamp_deployment.py --prefix build/``
...
def stamp(prefix: Path, build_root: Path) -> dict:
    ...
    Args:
        ...
        build_root: The zig build root, used to resolve a relative
            ``solver_dir`` recorded in the manifest (default ``runtime/``).
...
def main() -> None:
    ...
    parser.add_argument(
        "--build-root",
        default="runtime",
        help="zig build root for resolving a relative solver_dir (default: runtime)",
    )
