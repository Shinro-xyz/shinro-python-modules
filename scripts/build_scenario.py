...
``scripts/gen_scenario.py`` (``graph_data.zig`` + its manifest), compiles the
comptime VM via ``runtime/build.zig``, and — when ``--scenario`` is given —
verifies the result before stamping:
...
3. **Build** — ``zig build -Dgraph=<abs path>`` into an isolated prefix, so the
   shared ``runtime/graph_data.zig`` is never touched.
...
Without ``--scenario`` the script builds and stamps only — no oracle, no
integrity check — and prints a loud warning that the artifact is unverified.

Run::

    python3 scripts/build_scenario.py build/base --scenario tests/integration/scenarios/base_tracking.toml
...
try:
    from gen_scenario import gen_scenario, load_scenario
except ImportError:  # imported as scripts.build_scenario (e.g. from tests)
    from scripts.gen_scenario import gen_scenario, load_scenario

try:
    from stamp_deployment import stamp
except ImportError:
...
def main() -> int:
    ...
    optimize, target, solver_dir = "debug", "native", None
    if args.scenario:
        try:
            ...
            return EXIT_USAGE
        optimize = spec["compile"]["optimize"]
        target = spec["compile"]["target"]
        solver_dir = spec["compile"]["solver_dir"]
    if args.optimize:
    ...
    if manifest["has_solve_qp"] and not solver_dir:
        print(
    ...
    prefix = Path(args.prefix) if args.prefix else graph_dir

    try:
        _build(graph_path, prefix, optimize, target, solver_dir)
    ...
    if args.scenario:
        try:
            fresh_cg = _check_graph_integrity(args.scenario, graph_dir)
        except (ValueError, FileNotFoundError) as e:
            print(f"ERROR: {e}", file=sys.stderr)
            return EXIT_USAGE
        ...
        lib = _load_so(prefix)
        max_err = _oracle(lib, fresh_cg, args.samples, args.seed, tol)
        if max_err >= tol:
            print(
            ...
            return EXIT_ORACLE
        print(f"oracle B (.so vs interpret): {args.samples} random inputs, max abs err {max_err:.3e} ✓")
    else:
        print(
    ...
    stamp(prefix, RUNTIME)
    record = prefix / "lib" / "libbase.deployment.json"
